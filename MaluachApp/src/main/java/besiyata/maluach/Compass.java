...
public class Compass extends View implements SensorEventListener {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        Path p = new Path();
        float minwh=Math.min(getWidth(),getHeight())*0.98f;
        float Radius=minwh/2.0f;
        p.moveTo((-0.06f*Radius),0);
        p.lineTo( (0.06f*Radius),0);
        p.lineTo(0, 0.9f*Radius);
        p.lineTo( (-0.06f*Radius),0);
        ...
        canvas.save();
        {
            canvas.translate(getWidth() / 2, getHeight() / 2);
            {
                ...
                float font_height=bounds.height()/2;
